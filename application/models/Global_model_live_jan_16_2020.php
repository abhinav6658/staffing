...
class Global_model extends CI_Model{
	...
	public function miss_check_in($params = array()){
		...
		$this->db->where('s.start_date',$today);
		$this->db->where('s.user_id',$agency_id);
		$this->db->where('s.start_time <',$t_time);
	...
	public function check_already_added_schedule($params = array(),$end_date) {
		...
		$this->db->where('s.end_time >= "' . $end_time . '"');
		...
		if ($query->num_rows() >= 1) {
			if($count==0){
				...
				foreach ($query_results as $query_result) {
					if($query_result['start_date'] == date('Y-m-d')) {
						if(strtotime($query_result['start_time'])>strtotime(date('H:i:s'))) {
							continue 1;
						}
					}
					$count++;
	...
	/*Code Added by Prashant Kumar on 09 Jan 2020*/

	public function check_after_already_added_schedule($params = array(),$end_date) {
		$start_date = date('Y-m-d',strtotime($params['start_date']));
		$end_date 	= $end_date;
		//$agency_id 	= $params['user_id'];
		$start_time = date('H:i:s', strtotime($params['start_time']));
	...
	public function check_before_already_added_schedule($params = array(),$end_date) {
		$start_date = date('Y-m-d',strtotime($params['start_date']));
		$end_date 	= $end_date;
		//$agency_id 	= $params['user_id'];
		$start_time = date('H:i:s', strtotime($params['start_time']));
	...
	public function check_after_before_already_added_schedule($params = array(),$end_date) {
		$start_date = date('Y-m-d',strtotime($params['start_date']));
		$end_date 	= $end_date;
		//$agency_id 	= $params['user_id'];
		$start_time = date('H:i:s', strtotime($params['start_time']));
