...
*/
class Timesheetc2c extends CI_Controller
{
	...
	public function index() {
		...
		$vendid = $vendortdata[0]['guid'];

		$data['consultant_data'] = $this->global_model->get_data('panel_consultanttbl', array('vendor_id'=> $vendid, 'order_by' => 'consultant_id', 'cond' => 'DESC'));
		$data['title']           = 'Timesheet';
		$this->templated->load('Backend/c2c/default_layout', 'contents', 'timesheet/timesheet_c2c', $data);
	}
	...
	public function delete_timesheet() {
		if($this->input->get('id')){
			...
			$this->global_model->del_data('panel_timesheettbl',array("t_id"=>$this->input->get('id')));
									...
									redirect(base_url('timesheet-report-c2c'),'location');
								} else {
	...
	public function timesheet_report_c2c($report = null) {
		...
		$data['con_data']   = $this->global_model->get_data('panel_master_timesheettbl', array('vendor_id'=> $vendid, 'order_by' => 'id'));
		...
		$this->templated->load('Backend/c2c/default_layout', 'contents', 'timesheet/timesheet_report_c2c', $data);
	}

	

	public function timesheet_upload_c2c() {
			...
			//print_r($result);
				...
				if($var == '1') {
					...
					if($this->uri->segment(3) != '') {
						...
						$this->session->set_flashdata('error_message',$error_message);  
						redirect('pending-timesheet-c2c');
					} else {
						$error_message = 'Timesheet data already exists.Please try again.';
						$this->session->set_flashdata('error_message',$error_message);  
						redirect('timesheet-c2c');
					}
					//exit();
				} else {
	...
	public function getPendingTimesheet_c2c() {
		...
		$this->templated->load('Backend/c2c/default_layout', 'contents', 'timesheet/consultlist_pen_timesheet_c2c', $data);
	}
	public function pending_timesheet_c2c() {
		...
		//echo $start_date;
		//echo $end_date;
		$this->templated->load('Backend/c2c/default_layout', 'contents', 'timesheet/upload_pending_timesheet_c2c', $data);
	}
	else
	{
