...
Class Consultantsc2c extends CI_Controller {
	...
	public function index($report = null) {
		...
		$all_vendors  = $this->global_model->get_data('panel_vendortbl', array('email'=> $email, 'order_by' => 'vendor_id'));
		 ...
		 $data['con_data'] = $this->global_model->get_data('panel_consultanttbl', array('vendor_id'=> $vendid, 'order_by' => 'consultant_id'));

		 // print_r($data['con_data']); die;
		...
		$this->templated->load('Backend/c2c/default_layout', 'contents', 'consultants/consultant_list_c2c', $data);
	}
