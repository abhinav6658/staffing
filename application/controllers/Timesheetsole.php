...
*/
class Timesheetsole extends CI_Controller
{
	...
	public function index() {
		...
		$data['consultant_data'] = $this->global_model->get_data('panel_consultanttbl', array('email'=> $email, 'order_by' => 'consultant_id'));
		$data['title']           = 'Timesheet';
		$this->templated->load('Backend/sole/default_layout', 'contents', 'timesheet/timesheet_sole', $data);
	}
		...
		if($this->input->get('id')){
           ...
           delete_files($path, true , false, 1);
			...
			$this->global_model->del_data('panel_timesheettbl',array("t_id"=>$this->input->get('id')));
									...
									redirect(base_url('timesheet-report-sole'),'location');
								} else {
	...
	public function timesheet_report_sole($report = null) {
		...
		$data['con_data']   = $this->global_model->get_data('panel_master_timesheettbl', array('consultant_id'=> $consultdata[0]['guid'], 'order_by' => 'id'));
		...
		$this->templated->load('Backend/sole/default_layout', 'contents', 'timesheet/timesheet_report_sole', $data);
	}

		public function timesheet_upload_sole() {
		...
		public function getPendingTimesheet_sole() {
		...
		$this->templated->load('Backend/sole/default_layout', 'contents', 'timesheet/consultlist_pen_timesheet', $data);
	}
	public function pending_timesheet_sole() {
		...
		//echo $start_date;
		//echo $end_date;
		$this->templated->load('Backend/sole/default_layout', 'contents', 'timesheet/upload_pending_timesheet_sole', $data);
	}
	else
	{
