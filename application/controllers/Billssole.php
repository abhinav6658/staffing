...
*/
class Billssole extends CI_Controller
{
	...
	public function index() {
		...
		/* Code written by Ruchi Acharya*/
		$this->templated->load('Backend/sole/default_layout', 'contents', 'bills/billssole', $data);
	}
	...
	public function delete_bills() {
		...
		// $this->global_model->updatebill_status('panel_master_timesheettbl','bill_generated'=>'0',array("id"=>$this->input->get('id')));
					...
					$this->global_model->updateBillByIds($id);
									...
									redirect(base_url('bills-sole-report'),'location');
								} else {
	...
	public function bills_report() {
	    ...
	    $data['consult_data'] = $this->global_model->get_data('panel_consultanttbl', array('email'=> $email, 'order_by' => 'consultant_id'));

	     $consultid = $data['consult_data'][0]['guid']; 

		/* Code written by Ruchi Acharya*/

		$allbills_data = $this->global_model->get_data('panel_master_billstbl', array('consultant_id'=>$consultid, 'order_by' => 'bill_id', 'cond' => 'DESC'));
		...
		/* Code written by Ruchi Acharya*/
		$this->templated->load('Backend/sole/default_layout', 'contents', 'bills/bills_sole_report', $data);
	}
			...
			if($fileExt == 'pdf' || $fileExt == 'docx' || $fileExt == 'doc' || $fileExt == 'xlsx' || $fileExt == 'csv' || $fileExt == 'xls') {
				...
				if($bill['insert_id'] != "") {
					...
					if($this->uri->segment(3) != '') {
						redirect('pending-sole-bills/'. $this->uri->segment(3));
					} else {
						redirect('billssole');
					}			
	...
	public function getTimesheetHrs() {
		...
		$selected_consultantId 	= $_GET['consultant_id']; 
		 //echo $selected_consultantId;  //935B5B71A8714DF39B9E1D0D3F3EAD0B
		 //echo $selected_vendorId; die; //935B5B71A8714DF39B9E1D0D3F3EAD0Bw2
	...
	public function getConsultants() {
		...
		if($sel_vendor == 'w2') {
			...
			$consults = $this->global_model->get_data('panel_consultanttbl', array('email'=> $email, 'order_by' => 'consultant_id'));
	...
	$this->templated->load('Backend/sole/default_layout', 'contents', 'bills/consultlistsole_pen_bills', $data);
	}
	public function pending_bills() {
		...
		$this->templated->load('Backend/sole/default_layout', 'contents', 'bills/pending_sole_bills', $data);
	}
	public function upload_pending_bills() {
		...
		//print_r($data);
		$this->templated->load('Backend/sole/default_layout', 'contents', 'bills/upload_sole_pending_bills', $data);
