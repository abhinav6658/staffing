...
class Bills extends CI_Controller
{
	...
	public function index() {
		...
		/* Code written by Ruchi Acharya*/
		$this->templated->load('Backend/default_layout', 'contents', 'bills/bills', $data);
	}
	...
	public function delete_bills() {
		...
		//$this->global_model->updatebill_status('panel_master_timesheettbl','bill_generated'=>'0',array("id"=>$this->input->get('id')));
					...
					$this->global_model->updateBillByIds($id);
									...
									redirect(base_url('bills-report'),'location');
								} else {
	...
	public function bills_report() {
		...
		/* Code written by Ruchi Acharya*/
		$this->templated->load('Backend/default_layout', 'contents', 'bills/bills_report', $data);
	}
			...
			if($fileExt == 'pdf' || $fileExt == 'docx' || $fileExt == 'doc' || $fileExt == 'xlsx' || $fileExt == 'csv' || $fileExt == 'xls' || $fileExt == 'jpeg' || $fileExt == 'jpg' || $fileExt == 'png' || $fileExt == 'JPG' || $fileExt == 'JPEG' || $fileExt == 'PNG') {
				...
				if($bill['insert_id'] != "") {
					...
					if($this->uri->segment(3) != '') {
						redirect('pending-bills/'. $this->uri->segment(3));
					} else {
						redirect('bills');
					}			
	...
	public function getPendingBills() {
		...
		$this->templated->load('Backend/default_layout', 'contents', 'bills/consultlist_pen_bills', $data);
	}
	public function pending_bills() {
		...
		$this->templated->load('Backend/default_layout', 'contents', 'bills/pending_bills', $data);
	}
	public function upload_pending_bills() {
		...
		//print_r($data);
		$this->templated->load('Backend/default_layout', 'contents', 'bills/upload_pending_bills', $data);
