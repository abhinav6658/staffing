...
<div class="heading-noborder">
	<font color="#C97B29"><?php echo $breathcum; ?></font>	
</div>
...
<h2 class="infi-heading"> 
	...
	</span></div> <span class="rightAddbtn"><a href="add-vendor"><img src="<?php echo base_url('assets/'); ?>/images/icon/add.svg" alt="Add Icon"></a></span>
				...
				<tbody id="vendorList">
							...
							foreach($vendor_data as $vendor_value) {
								foreach ($vendor_con1 as $vendor_con_val1) {
									...
									if($vendor_value['guid'] == $vendor_con_val1['vendor_id']) {
										...
										$contact1_ext = $vendor_con_val1['extension'];
										break;
									} else {
										$contact1_name  = '';
										$contact1_email = '';
								...
								foreach ($vendor_con2 as $key => $vendor_con_val2) {
									if($vendor_value['guid'] == $vendor_con_val2['vendor_id']) {
										...
										$contact2_ext = $vendor_con_val2['extension'];
										break;
									} else {
										$contact2_name  = '';
										$contact2_email = '';
					...
					?>
								<tr>
									...
									<td width="15%"><?php echo $vendor_value['phone']; ?> (ext - <?php echo $vendor_value['extension']; ?>)</td>
									<td width="32%">
										<table class="table inner-table">
											<tr>
												<?php if($contact1_name != "") { ?>
														<td width="20%"><?php echo $contact1_name; ?></td>
												<?php } else {
												?>
														<td width="20%"></td>
												<?php } if($contact1_email != "") { ?>
														<td width="50%"><?php echo $contact1_email; ?></td>
												<?php } else {
												?>
														<td width="50%"></td>
												<?php } if($contact1_phone != "") { ?>
														<td width="30%"><?php echo $contact1_phone; ?>
															(ext - <?php echo $contact1_ext; ?>)
														</td>
												<?php } else {
												?>
														<td width="30%"></td>
										...
										<table class="table inner-table">
											<tr>
												<?php if($contact2_name != "") { ?>
														<td width="20%"><?php echo $contact2_name; ?></td>
												<?php } else {
													?>
														<td width="20%"></td>
												<?php } if($contact2_email != "") { ?>
														<td width="50%"><?php echo $contact2_email; ?></td>
												<?php } else {
												?>
														<td width="50%"></td>
												<?php } if($contact2_phone != "") { ?>
														<td width="30%"><?php echo $contact2_phone; ?> (ext - <?php echo $contact2_ext; ?>) </td>
												<?php } else {
												?>
														<td width="30%"></td>
												<?php
									...
									<td width="5%">
										<ul class="table-actionBtn">
											...
											<!-- <li><a href="<?php //echo base_url('add-contact'); ?>/<?php //echo $vendor_value['guid'] ?>"><img src="<?php //echo base_url('assets/') ?>images/icon/add.svg" alt="Document"></a></li> -->
...
jQuery("#vendor_id").change(function() {
	...
	if(filter_sel != "") {
		window.location.href = "<?php echo base_url('vendor-list'); ?>?ven=" + filter_sel;
	} else {
		window.location.href = "<?php echo base_url('vendor-list'); ?>?ven=" + filter_sel;
	}
});
