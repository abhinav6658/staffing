...
function get_email_template_language($template_slug, $email) {
    ...
    if (total_rows(db_prefix() . 'contacts', ['email' => $email,]) > 0 && in_array($template_slug, get_client_email_templates_slugs())) {
    ...
    } elseif (total_rows(db_prefix() . 'staff', ['email' => $email,]) > 0 && in_array($template_slug, get_staff_email_templates_slugs())) {
        $CI->db->where('email', $email);
        ...
        if (defined('EMAIL_TEMPLATE_PROPOSAL_ID_HELP')) {
            ...
            if ($class->get_rel_type() == 'proposal') {
                $CI->db->select('rel_type,rel_id')->where('id', $class->get_rel_id());
                $proposal = $CI->db->get(db_prefix() . 'proposals')->row();
            } elseif ($class->get_rel_type() == 'lead') {
                $CI->db->select('id, default_language')->where('id', $class->get_rel_id());
...
if (!function_exists('get_table_items_and_taxes')) {
    ...
    function get_table_items_and_taxes($items, $type, $admin_preview = false) {
        ...
        foreach ($items as $item) {
            ...
            $item_amount_with_quantity = hooks()->apply_filters('item_preview_amount_with_currency', app_format_number(($item['qty'] * $item['rate'])), ['item'       => $item, 'item_taxes' => $item_taxes,]);
