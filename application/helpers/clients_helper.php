...
function is_contact_email_verified($id = null) {
    ...
    $contact = $CI->db->get(db_prefix() . 'membertbl')->row();
...
function app_init_customer_profile_tabs() {
    ...
    $CI->app_tabs->add_customer_profile_tab('contacts', [
        'name'     => !is_empty_customer_company($client_id) || empty($client_id) ? _l('Users') : _l('Users'),
        'icon'     => 'fa fa-users',
...
/*function get_contact_permissions()
{
    $permissions = [
        [
            'id'         => 1,
            'name'       => _l('customer_permission_invoicez'),
            'short_name' => 'invoices',
        ],
        [
            'id'         => 2,
...
/* @Rakesh 4 Oct
 * Autorization Exist or not in DB
 *
 */

function get_authorization_activity_id($usersId = null,$usertype=null,$medicateProviderId=null) {
    $CI = & get_instance();
    // $CI->db->where($where);
    // return $CI->db->get(db_prefix() . 'shared_customer_files')->result_array();

    $CI->db->select('id');
    $CI->db->from('tblkorskill_acitivity');
    $CI->db->where('medicateProviderId', $medicateProviderId);
    $CI->db->where('user_id', $usersId);
...
function get_authorization_activity($usersId = null,$usertype=null,$medicateProviderId=null) {
    $CI = & get_instance();
    // $CI->db->where($where);
    // return $CI->db->get(db_prefix() . 'shared_customer_files')->result_array();
...
function get_authorization_name($id) {
    $CI = & get_instance();
    // $CI->db->where($where);
    // return $CI->db->get(db_prefix() . 'shared_customer_files')->result_array();

    $CI->db->select('name');
    $CI->db->from('tblkor_medicaid_provider');
    $CI->db->where('id', $id);
    $id = $CI->db->get()->row();
    //echo $CI->db->last_query();exit;
    if($id->name !=null) {
