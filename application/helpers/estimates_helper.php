...
function get_estimates_where_sql_for_staff($staff_id) {
    ...
    if ($has_permission_view_own) {
        $whereUser = '(('.db_prefix().'estimates.addedfrom=' . $staff_id . ' AND '.db_prefix().'estimates.addedfrom IN (SELECT staffid FROM '.db_prefix().'staff_permissions JOIN '.db_prefix().'permissions ON '.db_prefix().'permissions.permissionid='.db_prefix().'staff_permissions.permissionid WHERE '.db_prefix().'permissions.name = "estimates" AND can_view_own=1))';
