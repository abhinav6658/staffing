...
function get_proposals_sql_where_staff($staff_id) {
    ...
    $whereUser                          = '';
    if ($has_permission_view_own) {
        $whereUser = '(('.db_prefix().'proposals.addedfrom=' . $staff_id . ' AND '.db_prefix().'proposals.addedfrom IN (SELECT staffid FROM '.db_prefix().'staff_permissions JOIN '.db_prefix().'permissions ON '.db_prefix().'permissions.permissionid='.db_prefix().'staff_permissions.permissionid WHERE '.db_prefix().'permissions.name = "proposals" AND can_view_own=1))';
