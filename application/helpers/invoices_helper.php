...
function update_invoice_status($id, $force_update = false, $prevent_logging = false) {
    ...
    $CI->db->select('amount')->where('invoiceid', $id)->order_by(db_prefix() . 'invoicepaymentrecords.id', 'asc');
    ...
    $CI->db->where('id', $id);
    $CI->db->update(db_prefix() . 'invoices', ['status' => $status,]);
...
function get_invoices_where_sql_for_staff($staff_id) {
    ...
    if ($has_permission_view_own) {
        $whereUser = '((' . db_prefix() . 'invoices.addedfrom=' . $staff_id . ' AND ' . db_prefix() . 'invoices.addedfrom IN (SELECT staffid FROM ' . db_prefix() . 'staff_permissions JOIN ' . db_prefix() . 'permissions ON ' . db_prefix() . 'permissions.permissionid=' . db_prefix() . 'staff_permissions.permissionid WHERE ' . db_prefix() . 'permissions.name = "invoices" AND can_view_own=1))';
